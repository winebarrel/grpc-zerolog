...
func TestZerologPayloadSuite(t *testing.T) {
	...
	b.InterceptorTestSuite.ClientOpts = []grpc.DialOption{
		grpc.WithUnaryInterceptor(grpc_zerolog.PayloadUnaryClientInterceptor(b.logger, alwaysLoggingDeciderClient)),
		grpc.WithStreamInterceptor(grpc_zerolog.PayloadStreamClientInterceptor(b.logger, alwaysLoggingDeciderClient)),
	}
	nullLogger := zerolog.Nop()
	b.InterceptorTestSuite.ServerOpts = []grpc.ServerOption{
		grpc_middleware.WithStreamServerChain(
			grpc_ctxtags.StreamServerInterceptor(grpc_ctxtags.WithFieldExtractor(grpc_ctxtags.CodeGenRequestFieldExtractor)),
			grpc_zerolog.StreamServerInterceptor(&nullLogger),
			grpc_zerolog.PayloadStreamServerInterceptor(b.logger, alwaysLoggingDeciderServer)),
		grpc_middleware.WithUnaryServerChain(
			grpc_ctxtags.UnaryServerInterceptor(grpc_ctxtags.WithFieldExtractor(grpc_ctxtags.CodeGenRequestFieldExtractor)),
			grpc_zerolog.UnaryServerInterceptor(&nullLogger),
			grpc_zerolog.PayloadUnaryServerInterceptor(b.logger, alwaysLoggingDeciderServer)),
	}
